...
pub const IpAddress = union(enum) {
    ...
    /// Converts from a pre-0.16 std.net.Address.
    pub fn fromStdAddress(addr: *const std.net.Address) IpAddress {
        return switch (addr.any.family) {
            std.posix.AF.INET => .{ .ip4 = .{
                .bytes = @bitCast(addr.in.sa.addr),
                .port = std.mem.bigToNative(u16, addr.in.sa.port),
            } },
            std.posix.AF.INET6 => .{ .ip6 = .{
                .bytes = addr.in6.sa.addr,
                .port = std.mem.bigToNative(u16, addr.in6.sa.port),
                .flowinfo = std.mem.bigToNative(u32, addr.in6.sa.flowinfo),
                .scope_id = std.mem.bigToNative(u32, addr.in6.sa.scope_id),
            } },
            else => unreachable,
        };
    }

    /// Converts to a pre-0.16 std.net.Address for use with legacy APIs.
    pub fn toStdAddress(self: IpAddress) @import("std").net.Address {
        return switch (self) {
            .ip4 => |ip4| .{ .in = .{ .sa = .{
                .port = @import("std").mem.nativeToBig(u16, ip4.port),
                .addr = @bitCast(ip4.bytes),
            } } },
            .ip6 => |ip6| .{ .in6 = .{ .sa = .{
                .port = @import("std").mem.nativeToBig(u16, ip6.port),
                .flowinfo = @import("std").mem.nativeToBig(u32, ip6.flowinfo),
                .addr = ip6.bytes,
                .scope_id = @import("std").mem.nativeToBig(u32, ip6.scope_id),
            } } },
        };
    }
...
pub const ShutdownError = error{
    ...
    SocketUnconnected,
    SystemResources,
} || error{Unexpected} || error{Cancelable};
...
pub const Socket = struct {
    ...
    pub const Handle = if (@import("builtin").os.tag == .windows) std.os.windows.ws2_32.SOCKET else std.posix.fd_t;
