    ...
    if (builtin.os.tag == .windows) {
        switch (clock) {
            .real => {
                ...
                // FILETIME is in 100ns ticks since 1601-01-01, convert to ns since 1970-01-01.
                const ticks = (@as(u64, ft.dwHighDateTime) << 32) | ft.dwLowDateTime;
                const ns_since_1601: i96 = @as(i96, ticks) * 100;
                const ns_1601_to_1970: i96 = 11644473600 * std15.time.ns_per_s;
                return .{ .nanoseconds = ns_since_1601 - ns_1601_to_1970 };
            },
            .awake, .boot => {
        ...
        const clock_id: posix.clockid_t = switch (clock) {
